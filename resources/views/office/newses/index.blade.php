...
@section('meta')
<title>お知らせ一覧 | {{ config('app.name') }}</title>
@endsection
                            ...
                            <form method="GET" action="" class="">
                                ...
                                <div id="collapseSearch" class="accordion-collapse collapse {{ request()->accordion ? 'show' : '' }}" aria-labelledby="headingSearch" data-bs-parent="#accordionSearchArea">
                                    <div class="accordion-body p-0">
                                        ...
                                        <div class="row">
                                            <div class="col-12">
                                                <a href="{{ route('officeNewsIndex') }}" class="btn btn-outline-dark w-100 rounded-2 mt-3 py-1">検索条件をクリアする</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $record->status == 0 ? 'bg-lighter' : '' }}">
                                    ...
                                    <td class="text-center py-2">
                                        ...
                                        <a href="{{ route('officeNewsShow', ['id' => $record->id]) }}" class="btn btn-sm btn-icon btn-outline-info me-2" title="詳細">
                                            <i class="bx bx-xs bx-info-square"></i>
                                        </a>
                                        <a href="{{ route('officeNewsEditInput', ['id' => $record->id]) }}" class="btn btn-sm btn-icon btn-outline-warning me-2" title="編集">
                                            <i class="bx bx-xs bxs-pencil"></i>
                                        </a>
                                        <form method="POST" action="{{ App\Libraries\Utils::urlToPath(route('officeNewsDeleteExecute', ['id' => $record->id])) }}" enctype="multipart/form-data" class="d-inline" onsubmit="return confirmDelete()">
                                            @csrf
